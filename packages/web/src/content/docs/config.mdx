...
The `fallback` option configures a model (in the format of `provider/model`, eg `"fallback": "ollama/qwen3"`) that the session automatically switches to when the primary model fails with a retryable transport error — for example when your connection drops mid-session. The session keeps working locally instead of stalling, and the switch is announced in the conversation.

The `image_budget` option caps how many images are sent per model request (default 40). Oldest images beyond the budget are replaced with a text placeholder while the newest are kept, so long sessions with many screenshots cannot die on provider image-per-request limits.
...
- `headerTimeout` - Timeout in milliseconds to wait for response headers (default: 60000, or 1 minute). This timer stops once headers arrive and does not limit the streamed response body. Set to `false` to disable.
- `chunkTimeout` - Timeout in milliseconds between streamed response chunks (default: 60000, or 1 minute). If no chunk arrives in time, the request is aborted. Set to `false` to disable.
